...
{reading, cls, status, context}`, `contested`, `contestedNote`, `tags[]`. The
prototype's schema had no slot for alternates, and dropping them would break
contested-reading display, which is a design principle; `chars[].on`, `.kun`,
`.note`, `contested`, `contestedNote` and `tags` are additive fields the
inspection page and the Phase 1 modes need, each derived one-to-one from a
canonical field.
