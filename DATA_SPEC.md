...
| `data/source/compound-drill.prototype.jsx` | The React prototype; its compact entries are extracted from `data/source/compound-drill.prototype.jsx` at consolidation time | **Never.** Read-only. |
...
`compound`, `reading`, `chars[] {k, r, t, on, kun, note}`, `cls`, `diff`,
`changes`, `changeDetail`, `context`, `trap`, `chains[]`, `alternates[]
{reading, cls, status, context, sourceStatus}`, `contested`, `contestedNote`,
`tags[]`. The prototype's schema had no slot for alternates, and dropping them
would break contested-reading display, which is a design principle;
`chars[].on`, `.kun`, `.note`, `alternates[].sourceStatus` (the source's status
text verbatim, §2.2), `contested`, `contestedNote` and `tags` are additive
fields the inspection page and the Phase 1 modes need, each derived one-to-one
from a canonical field.

Not emitted: `source_id`, `source_batch`, `char_count`, `has_kana`,
`decomposable` (consolidation-only) and `difficulty_rationale`.

Round-trip test (`scripts/build-compact.test.ts`): for every generated compact
entry, each field listed above must equal the canonical field it derives from,
and the canonical entry minus the six fields not emitted must be reconstructible
from the compact entry. This is a cheap test that catches transform drift
immediately.
...
| contains "widespread but contested", "prescriptively contested", or "originally non-standard" | `variant_spreading` |
