...
- The repository `isleofdan/compound-readings` exists (private, default branch `main`) with the ten planning and source files from the Session 1 zip installed byte for byte; everything under `data/source/` is read-only history and is never edited.
...
- The GitHub Pages deploy workflow is committed and its build job passes in Actions, but Pages itself is **not enabled** (enabling it was blocked on the laptop; it is Dan's action), so nothing is live yet.
...
- **Pages:** not live. Enabling it through the GitHub API was blocked by Claude Code's permission check on the laptop, so the private-repo/plan question was never reached. The workflow's deploy job fails on every push until Pages is enabled.
